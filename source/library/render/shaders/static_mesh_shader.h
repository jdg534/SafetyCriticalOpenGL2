...
void main()
{
    ...
    vs_out_world_position = world_pos.xyz;
...
uniform vec3      u_ambient_light_color; // light colour
uniform vec4      u_surface_tint;        // color multiplier
...
void main()
{
    ...
    vec3 ambient_light_colour = u_ambient_light_color;
    vec3 diffuse_light_colour = u_lightColor * normal_dot_to_light;

    vec3 combined_light_colour = ambient_light_colour + diffuse_light_colour;
    vec4 surface_colour = vec4(diffuse_surface_colour.rgb * lit, diffuse_surface_colour.a)
                        * u_surface_tint;
    if (surface_colour.a <= 0.0) discard;
